...
def demo(filepath):
    ...
    # stream = SEAGenerator(sample_seed=random.randint(0, 1000), noise_percentage=0.10)
    ...
    ensemble = EnsembleVoteClassifier(voting='sum_prob', classes=np.asarray(stream.get_targets()), window_slide=batch_size, clfs=[
        GaussianNB()
        # ,BernoulliNB()
