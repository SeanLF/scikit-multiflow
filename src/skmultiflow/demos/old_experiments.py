...
for percentage in [100,90,80,70,60]:
    ...
    thesis_experiment(experiment_name, classifier=Classifier.VOTING_ENSEMBLE, voting=Voting.SUM_PROB, g_t_percentage=percentage)
...
thesis_experiment('voting_type_drift/sum_dw', classifier=Classifier.VOTING_ENSEMBLE, voting=Voting.SOFT)#, drift=drift) # default strategy
thesis_experiment('voting_type_drift/sum_duw', classifier=Classifier.VOTING_ENSEMBLE, voting=Voting.SUM_PROB)#, drift={'drift_reset': DriftReset.ALL, 'g_t_%': 80, 'drift_detection_method': 'one_proba', 'drift_use_weighted_probabilities': False})
thesis_experiment('voting_type_drift/soft', classifier=Classifier.VOTING_ENSEMBLE, voting=Voting.SOFT, drift={'drift_reset': DriftReset.ALL, 'g_t_%': 80, 'drift_detection_method': 'one_proba', 'drift_use_weighted_probabilities': False})
thesis_experiment('voting_type_drift/hard', classifier=Classifier.VOTING_ENSEMBLE, voting=Voting.HARD, drift={'drift_reset': DriftReset.ALL, 'g_t_%': 80, 'drift_detection_method': 'boolean', 'drift_use_weighted_probabilities': False})
