...
if p < ALPHA[0]:
    ...
    pn, ranks, qs, qa = sp.posthoc_nemenyi_friedman(df, sort=True, melted=True, y_col='y', block_col='blocks', group_col='groups')
    if len(np.argwhere(np.logical_and(pn.values <= ALPHA[0], pn.values >= 0))) > 0: # check if at least 1 value has p < 0.05
        ...
        for alpha in ALPHA:
            ...
            if len(np.argwhere(np.logical_and(pn.values <= alpha, pn.values >= 0))) > 0:
                ...
                reverse = True if params['measure'] == 'kappa_t' else False
                ...
                Orange.evaluation.graph_ranks(avranks, names, cd=cd, width=15, textspace=2, reverse=reverse, filename=filename)
                plt.close(None)
    
    f = open('{}{}.txt'.format(params['dir'], params['measure']), 'w')
    dashes = ('-')*23
    f.write("\nFriedman Statistic: {}\tp-value: {}\n\n\nPost-hoc Nemenyi\n\n\nRanks:\n{}\n{}\n\nCritical difference:\n{}\n{}\n\nDifference:\n{}\n{}\n\nTable:\n{}\n{}".format(str(stat),str(p), dashes,str(ranks),dashes,str(qa),dashes,str(qs.replace([0], '')),dashes,str(pn)))
    f.close()